...
// Uninstall uninstalls the specified release.
...
// Revisions returns the revision history for the specified release.
func Revisions(params RevisionsParameters) ([]storage.Release, error) {
	...
	hist, err := client.Run(params.Release)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	...
	releases := make([]storage.Release, len(hist))
